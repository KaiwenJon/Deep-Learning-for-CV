...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            # print("Training Discriminatror")
            # Train the discriminator
            D_solver.zero_grad()

            # Generate fake images and calculate discriminator loss
            noise = sample_noise(batch_size, noise_size).to(device)
            fake_images = G(noise)
            fake_images = fake_images.view_as(real_images)
            logits_real = D(real_images).squeeze()
            logits_fake = D(fake_images.detach()).squeeze()
            ...
            logits_fake = D(fake_images).squeeze()
